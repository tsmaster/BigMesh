...
class City:
    ...
    def __str__(self):
        return "%s, %s (%f, %f) p: %d" % (self.name, self.state_name, self.lat, self.lon, self.pop)
    ...
    if proc.returncode == 0:
        ...
        for line in out_lines:
            ...
            if line.startswith('Found a city:'):
                #print(line)
                m = re.search('name=\"([A-Za-z ]+)\" lat=([0-9\.-]*), lon=([0-9\.-]*), population=([0-9]+)', line)
                #print ("city match?", m)
                if m:
                    #print(m[1])
                    #print(m[2])
        ...
        for sc in state_cities:
            ...
            draw.ellipse((ix - radius, iy - radius,
                          ix + radius, iy + radius),
                         fill = (255, 192, 0),
                         )
            cities.append(sc)
