...
def lat_lon_to_wm_xy(lat, lon):
    ...
    y = (256/(2*math.pi))*(math.pi - math.log(math.tan(math.pi/4.0 + lat_rad / 2.0)))
...
def haversine_distance_deg(lat_1, lon_1, lat_2, lon_2):
    ...
    a = (sin_lat_over_two * sin_lat_over_two +
         cos_lat_1 * cos_lat_2 *
         sin_lon_over_two * sin_lon_over_two)
...
def haversine_offset_deg(lat, lon, heading, distance_deg):
    ...
    lat_2_r = math.asin(math.sin(lat_1_r) * math.cos(distance_r) +
                        math.cos(lat_1_r) * math.sin(distance_r) * math.cos(azimuth_r))
    lon_2_r = lon_1_r + math.atan2(math.sin(azimuth_r) * math.sin(distance_r) * math.cos(lat_1_r),
